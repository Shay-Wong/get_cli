...
class PubspecUtils {
  ...
  static bool updateGetCliYaml(String key, dynamic value, {bool logger = true}) {
    try {
      var yaml = pubSpec.unParsedYaml!;
      final entries = (yaml['get_cli'] as YamlMap?)?.entries;
      var newVaule = Map.fromEntries(entries ?? {})..[key] = value;
      var newPub = pubSpec.copy(
        unParsedYaml: yaml..["get_cli"] = newVaule,
      );
      _savePub(newPub);
    } on Exception catch (_) {
      return false;
    ...
    if (logger) {
      LogService.success(LocaleKeys.sucess_update_yaml.trArgs(['$key: $value']));
    }
    return true;
  }
